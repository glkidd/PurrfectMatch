...
CREATE TABLE `dogs` (
  ...
  `spayed_neutured` tinyint(1) NOT NULL,
...
CREATE TABLE `cats` (
  ...
  `breed` int(11) NOT NULL,
  `gender` enum('MALE','FEMALE','OTHER') NOT NULL,
  `furlength` enum('NOHAIR','SHORT','MEDIUM','LONG') NOT NULL,
  `bio` varchar(1000) NOT NULL,
  `shelterid` bigint(255) NOT NULL,
  `datearrived` date DEFAULT NULL,
  `euthanizedate` date DEFAULT NULL,
  `spayedneutured` tinyint(1) NOT NULL,
  PRIMARY KEY (`id`),
  KEY `shelterid` (`shelterid`),
  KEY `breed` (`breed`),
  CONSTRAINT `cats_shelter_id_fk` FOREIGN KEY (`shelterid`) REFERENCES `shelter_accounts` (`id`),
